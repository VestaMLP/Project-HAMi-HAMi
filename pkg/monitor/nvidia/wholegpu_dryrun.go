...
func RunWholeGPUDryRun(ctx context.Context, client kubernetes.Interface, nvmllib nvml.Interface, nodeName string, opts WholeGPUDryRunOptions) (*WholeGPUDryRunReport, error) {
	...
	for _, candidate := range candidates {
		verdict := evaluateContainerWholeGPU(candidate.devices, nodeDevs)
	...
	if report.ConfirmedContainers > 0 {
		...
		if !errors.Is(ret, nvml.SUCCESS) {
			return nil, fmt.Errorf("NVML initialization failed: %v", ret)
		}
...
func sortWholeGPUReport(report *WholeGPUDryRunReport) {
	...
	for i := range report.Containers {
		sort.Slice(report.Containers[i].Devices, func(left, right int) bool {
			return report.Containers[i].Devices[left].Index < report.Containers[i].Devices[right].Index
		})
	}
